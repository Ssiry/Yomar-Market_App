...
const CategoryProducts = () => {
    ...
    const [categoryName, setCategoryName] = useState('تصنيف');
    ...
    const fetchProducts = async () => {
        try {
            ...
            const categoryId = await AsyncStorage.getItem('SelectedCategoryId');
            setCategoryName(await AsyncStorage.getItem('SelectedCategoryName') || '');

            if (!marketId || !categoryId) {
                console.error('No market or Category ID found');
                return;
            }

            const response = await axios.get(`http://192.168.1.11:5007/market/products/by-category/${marketId}/${Number(categoryId)}`);
